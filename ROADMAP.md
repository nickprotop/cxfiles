...
_Nothing right now — pick the next item._

## Next up

### 1. Navigation polish — bookmarks + back/forward history

Two related features shipped together as one PR. Both are
Explorer-standard and unlock muscle memory most users already have.

**Back/forward history per tab.** Each `TabState` keeps two stacks of
visited paths. `Alt+Backspace` goes back, `Alt+Shift+Backspace` goes
forward. Two clickable arrows on the breadcrumb bar mirror the
keybinds. Search activation does not push history; clearing search
does not pop.

**User bookmarks.** `Ctrl+D` bookmarks the current folder. Bookmarks
appear as their own section in the folder tree (above _Folders_) and
are persisted in the existing config service. Right-click a bookmark
to rename or remove. The breadcrumb's existing built-in quick-access
(Home, Desktop, Documents, Downloads, Trash) stays — the user list
sits alongside it.

**Files to touch:** `App/TabState.cs`, `App/CXFilesApp.cs`,
`App/CXFilesApp.Keyboard.cs`, `UI/Components/FolderTreePanel.cs`,
`Services/IConfigService.cs` (new `Bookmarks` array).
